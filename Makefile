...
build-windows:
	...
	# SHARED
	cl /O2 /D_USRDLL /D_WINDLL /DRAYGUI_IMPLEMENTATION /DBUILD_LIBTYPE_SHARED raygui.c /LD /Febin/$(TARGET_PLATFORM)/raygui.dll /link /LIBPATHlib/$(TARGET_PLATFORM) msvcrt.lib winmm.lib gdi32.lib user32.lib shell32.lib /subsystem:windows
	mv bin/$(TARGET_PLATFORM)/raygui.lib bin/$(TARGET_PLATFORM)/rayguidll.lib

	# STATIC
	cl /c raygui.c /Fobin/$(TARGET_PLATFORM)/raygui.obj /LIBPATHlib/$(TARGET_PLATFORM) /DRAYGUI_IMPLEMENTATION
...
build-macos:
	...
	# SHARED
	gcc -O2 -dynamiclib -o bin/$(TARGET_PLATFORM)/libraygui.5.0.0.dylib raygui.c -DRAYGUI_IMPLEMENTATION -L lib/$(TARGET_PLATFORM) -lraylib -framework OpenGL -framework CoreFoundation -framework CoreGraphics -lm -lpthread -ldl

	# STATIC
	clang -O2 -c raygui.c -o bin/$(TARGET_PLATFORM)/raygui.o -DRAYGUI_IMPLEMENTATION -L lib/$(TARGET_PLATFORM) -lraylib -framework OpenGL -framework CoreFoundation -framework CoreGraphics -lm -lpthread -ldl
